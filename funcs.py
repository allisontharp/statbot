...
order by c.date desc, c.rank desc limit 1
    ...
    sql(c, "DETACH DATABASE 'bgcol'")
    
    return worst[0][0]

def usertopmech(c, userid): 
    query = '''
ATTACH DATABASE 'bgcol.sqlite' AS bgcol;'''
    ...
    query = '''
SELECT m.name, count(usercol.bggid) as cnt
...
ORDER BY cnt desc limit 10
    ...
    query = '''
DETACH DATABASE 'bgcol';'''
    
    sql(c, query)

    return out

def usertopcat(c, userid): 
    query = '''
ATTACH DATABASE 'bgcol.sqlite' AS bgcol;'''
    ...
    query = '''
SELECT cat.name, count(usercol.bggid) as cnt
...
ATTACH DATABASE 'bgcol.sqlite' AS bgcol;'''
    ...
    query = '''
SELECT count(distinct g.bggid), a.tot
FROM games g
INNER JOIN bgcol.collection c ON c.bggid = g.bggid
INNER JOIN collection col ON col.bggid = g.bggid
INNER JOIN (SELECT count(distinct c.bggid) as tot
            FROM collection c
            WHERE c.userid = {uid}) AS a 
WHERE c.rank >=1 AND c.rank <= {tp} AND col.userid = {uid};'''.format(tp = top, uid = userid)
...
def collectionmain(c, username, **kwargs):
    ...
    ### Top mechanics owned
    topmech = usertopmech(c, userid)
    
    head = ('Mechanic', 'Number Owned')
    topmech = [head] + topmech
    
    #print rtable(topmech)
    
    topcat = usertopcat(c, userid)
    head = ('Category', 'Number Owned')
    topcat = [head] + topcat
    #print rtable(topcat)
 
 
    out += "\n\n----\n\nA Summary of your most owned mechanics is below:\n\n\n"
    out += rtable(topmech)
    
    out += "\n\n----\n\nA Summary of your most owned categories is below:\n\n"
    out += rtable(topcat)
